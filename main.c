...
#define STEPER_PIN0 4
#define STEPER_PIN1 5
#define STEPER_PIN2 6
#define STEPER_PIN3 7


#define WIRE_BRAKE_SENSOR_PIN 0
...
uint16_t	qLays;			//	количество слоёв (требется намотать)
uint16_t	QTurns;			//	количество витков (требется намотать)
uint16_t	QTurnsInLay;	//	количество витков в слое (требется намотать)
...
CurrentValuesStruct EEMEM EECurrentValues[10];

CurrentValuesStruct SavedCurrentValues;


//CurrentValuesStruct EEMEM SavedCurrentValues[10];
...
{  //  Nom,  Next, Previous, Parent, Child, HandlerNum, Text
	...
	{ 2, 3, 1, 0, 0, HE_SAVE_MEM, "Save mem\0"},
	{ 3, 1, 2, 0, 0, HE_LOAD_MEM, "Load mem\0"},	
	...
	{11, 6,10, 4, 0, HE_SET_STEP_ENG_DIR_CORR,"Step dir cor.\0"},
...
const char ErorWireBreakDEscrition[] PROGMEM = "ERROR Wire Break\0";
const char StrMovingToA[] PROGMEM = "Moving to A\0";
const char StrMovingToB[] PROGMEM = "Moving to B\0";
const char StrPosAmemorized[] PROGMEM = "Pos. A memorized\0";
const char StrPosBmemorized[] PROGMEM = "Pos. B memorized\0";
const char StrDataSaving[] PROGMEM = "Data is saved\0";
const char StrDataLoading[] PROGMEM = "Data is loaded\0";
...
uint8_t FindLastEESaveValuesInd()
{
	CurrentValuesStruct SavedCurrentValues;
	
		uint8_t volatile ind = 0;
		eeprom_read_block(&SavedCurrentValues, &EECurrentValues[ind], sizeof(SavedCurrentValues));
		//uint8_t firstNom = SavedCurrentValues.Nom;
		uint8_t PrevNom = SavedCurrentValues.Nom;
		uint8_t Nom = 0;

		//ind++;
		// шаримся по массиву, смотрим индекс Nom. Если текущй не больше предыдущего или больше чем на 1, значит предыдущий был последним записанным.
		while(ind < 10)
		{
			eeprom_read_block(&SavedCurrentValues, &EECurrentValues[ind+1], sizeof(SavedCurrentValues));
			Nom = SavedCurrentValues.Nom;
						
			if ((Nom == (PrevNom+1)) || (Nom==0 && PrevNom==255))
			{
				PrevNom = Nom;
				ind++;
			}
		...
		// дошли до края массива, все инлексы по порядку возрастают, значит актуальный элемент - последний
		return (ind);
}

void ReadEESaveValues()
{
	CurrentValuesStruct SavedCurrentValues;
	uint8_t ind = 0;
	
	ind = FindLastEESaveValuesInd();
	
	eeprom_read_block(&SavedCurrentValues, &EECurrentValues[ind], sizeof(SavedCurrentValues));
		
	QTurns = SavedCurrentValues.QTurns;
	QTurnsInLay = SavedCurrentValues.QTurnsInLay;
	Step = SavedCurrentValues.Step;
	CurQTurns = SavedCurrentValues.CurQTurns;
	CurQTurnsInLay = SavedCurrentValues.CurQTurnsInLay;
	CurLay = SavedCurrentValues.CurLay;
	WindDir = SavedCurrentValues.WindDir;
	WindMode = SavedCurrentValues.WindMode;
	
	//NumCommaPos = 0;	// второй символ - запятая  0,...
	//NumLen = 3;
	//uint8_t PrintNum  = ind;
	//AddrNumForEdit = utoa_fast_div_len(PrintNum, NumForEditText, NumLen);
	//lcd_gotoxy(1, 1);
	//PrintNumComma(AddrNumForEdit, NumCommaPos, NumLen);
	//BlinkTimer = 2000;
		//
	//PrintNum  = SavedCurrentValues.Nom;
	//AddrNumForEdit = utoa_fast_div_len(PrintNum, NumForEditText, NumLen);
	//lcd_gotoxy(7, 1);
	//PrintNumComma(AddrNumForEdit, NumCommaPos, NumLen);
	//BlinkTimer = 1000;
...
void WriteEESaveValues()
{
	CurrentValuesStruct SavedCurrentValues;
	uint8_t volatile ind_ws = 0;
	
	ind_ws = FindLastEESaveValuesInd();
		
	eeprom_read_block(&SavedCurrentValues, &EECurrentValues[ind_ws], sizeof(SavedCurrentValues));
	
	uint8_t Nom = SavedCurrentValues.Nom;
	...
	SavedCurrentValues.QTurns = QTurns;
	SavedCurrentValues.QTurnsInLay = QTurnsInLay;
	SavedCurrentValues.Step = Step;
	SavedCurrentValues.CurQTurns = CurQTurns;
	SavedCurrentValues.CurQTurnsInLay = CurQTurnsInLay;
	SavedCurrentValues.CurLay = CurLay;
	SavedCurrentValues.WindDir = WindDir;
	SavedCurrentValues.WindMode = WindMode;
	SavedCurrentValues.Nom = Nom;
	
	eeprom_write_block(&SavedCurrentValues, &EECurrentValues[ind_ws], sizeof(SavedCurrentValues));
...
// Гоняем по таблице переходов MenuItems
uint8_t JmpMenu(uint8_t Nom, uint8_t Key)
{
	...
	switch (Key)
	{
		...
		break;
		//case KEY_MENU	:  // в корне меню кнопка KEY_MENU работает аналогично KEY_RET  
		//NewNomM = pgm_read_byte(&(MenuItems[ind].Parent));
		//if (NewNomM==0)
		//{
			//NewNom = NewNomM;
		//}
...
void Menu()
{
	...
	{
		// опрос клавиатуры
		KeyCode = GetKey();   // что там у нас нажат на клавиатуре
		EventCode = KeyEvent(KeyCode);   // и какое собвтие это означает
		...
		if (KeyCode==KEY_RET || KeyCode==KEY_MENU)  // выходим из текущего пункта меню
		{
			MenuState = 1; 
			lcd_clrscr(0); // 0 - весь экран
			return;
		}
		
		MenuState = JmpMenu(MenuState, KeyCode); // определяем точку перехода в меню
		ShowMenu(MenuState, 0);					 // отображение нового пункта меню	
		MenuKeyEventsHandling(HandlerNum);       // обработка события перехода в меню
	}
}
...

{
  ...
  //настраиваем условие прерывания
    MCUCR |= (1<<ISC00)|(0<<ISC00)|(1<<ISC11)|(0<<ISC10);
	...
	DDRC =  (1<<STEPER_PIN0)| (1<<STEPER_PIN1)|(1<<STEPER_PIN2)|(1<<STEPER_PIN3) & ~(1<<WIRE_BRAKE_SENSOR_PIN);    // переводит на вывод STEPER_PIN, на ввод WIRE_BRAKE_SENSOR_PIN
	PORTC = PORTC | (1<<WIRE_BRAKE_SENSOR_PIN); // включаем подтягивающий резистор
	Led = 0b11111111;
	//PORTC = Led;
	counttime = 0;
	KeyScanTimer = 0;
...
//{}
	...
	while (1)
	{
		...
		case KEY_OK:
			...
			lcd_cursor(2);
			break;
		case KEY_POS_STAKER_A:
			
			StepCounerA = StepCouner;
			CurIndField = indField;
			lcd_cursor(1);
			ShowMessage(&StrPosAmemorized, 7);
			lcd_clrscr(1);
			lcd_clrscr(2);
			ShowMainScreen();
			SetToField(CurIndField);
			lcd_cursor(2);
			break;	
		case KEY_POS_STAKER_B:
			StepCounerB = StepCouner;
			CurIndField = indField;
			lcd_cursor(1);
			ShowMessage(&StrPosBmemorized, 7);
			lcd_clrscr(1);
			lcd_clrscr(2);
			ShowMainScreen();
			SetToField(CurIndField);
			lcd_cursor(2);
			break;
		case KEY_MOVE_TO_POS_STAKER_A:
			CurIndField = indField;
			lcd_cursor(1);
			MoveToPosStaker(StepCounerA, &StrMovingToA);
			lcd_clrscr(1);
			lcd_clrscr(2);
			ShowMainScreen();
			SetToField(CurIndField);
			lcd_cursor(2);
			break;
		case KEY_MOVE_TO_POS_STAKER_B:
			CurIndField = indField;
			lcd_cursor(1);
			MoveToPosStaker(StepCounerB, &StrMovingToB);
			lcd_clrscr(1);
			lcd_clrscr(2);
			ShowMainScreen();
			SetToField(CurIndField);
...
{   
	...
	else
	{
		...
		{
			PulseCount = PulseCount + QPulsVal;
			if (CurQTurns>0 && CurQTurnsInLay>0)
			{
				CurQTurns--;
				CurQTurnsInLay --;
			}
